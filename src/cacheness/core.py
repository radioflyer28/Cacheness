...
class CacheConfig:
    ...
    hash_path_content: bool = True  # Hash Path objects by content; False uses filename only
...
class UnifiedCache:
    ...
    def __init__(self, cache_dir_or_config: Optional[Union[str, Path, CacheConfig]] = None, metadata_backend=None):
        """
        ...
        if isinstance(cache_dir_or_config, (str, Path)):
            ...
            raise TypeError(f"Expected str, Path, or CacheConfig, got {type(cache_dir_or_config)}")
        ...
        logger.info(f"✅ Unified cache initialized: {self.cache_dir} (backend: {self.actual_backend})")
    ...
    def put(self, data: Any, description: str = "", prefix: str = "", **kwargs):
        ...
        try:
            ...
            logger.info(f"Cached {handler.data_type} {cache_key} ({file_size_mb:.3f}MB) {format_info}: {description}")
            
        except Exception as e:
            logger.error(f"Failed to cache {handler.data_type}: {e}")
            raise
    
    def get(self, ttl_hours: Optional[int] = None, prefix: str = "", **kwargs) -> Optional[Any]:
        """
    ...
    def get_dataframe(self, ttl_hours: Optional[int] = None, prefix: str = "", **kwargs):
...
def get_cache(config: Optional[CacheConfig] = None, metadata_backend=None) -> UnifiedCache:
