...
class CacheEntrySigner:
    ...
    def __init__(
        ...
        logger.debug(
            f"Cache signer initialized: fields={self.signed_fields}, key_type={key_type}"
        )
    ...
    def sign_entry(self, entry_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Created signature for entry {entry_data.get('cache_key', 'unknown')}"
            )
            return signature
    ...
    def verify_entry(self, entry_data: Dict[str, Any], stored_signature: str) -> bool:
        ...
        try:
            ...
            if not is_valid:
                logger.warning(
                    f"Signature verification failed for entry {entry_data.get('cache_key', 'unknown')}"
                )
            else:
                logger.debug(
                    f"Signature verified for entry {entry_data.get('cache_key', 'unknown')}"
                )
