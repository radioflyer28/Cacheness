...
class CacheStorageConfig:
    ...
    create_cache_dir: bool = (
        True  # Automatically create cache directory if it doesn't exist
    )
...
class CacheMetadataConfig:
    ...
    metadata_backend: str = (
        "auto"  # "auto", "json", "sqlite", or custom registered backend name
    )
    metadata_backend_options: Optional[dict] = (
        None  # Backend-specific options (e.g., connection_url for postgresql)
    )
    ...
    enable_memory_cache: bool = (
        False  # Enable in-memory caching of disk-stored metadata entries
    )
    ...
    def __post_init__(self):
        ...
        if self.metadata_backend_options:
            logger.debug(
                f"Metadata backend options: {list(self.metadata_backend_options.keys())}"
            )
        if self.memory_cache_type and self.enable_memory_cache:
            logger.debug(
                f"Memory cache layer: {self.memory_cache_type} (maxsize={self.memory_cache_maxsize}, ttl={self.memory_cache_ttl_seconds}s)"
            )
...
class CacheBlobConfig:
    ...
    blob_backend: str = (
        "filesystem"  # "filesystem", "memory", or custom registered backend
    )
    ...
    stream_threshold_bytes: int = (
        10 * 1024 * 1024
    )  # 10MB - use streaming for larger blobs
    ...
    def __post_init__(self):
        ...
        if self.shard_chars > 8:
            raise ValueError(
                "shard_chars must be <= 8 (excessive sharding not recommended)"
            )

        logger.debug(f"Blob backend configured: {self.blob_backend}")
        if self.blob_backend_options:
            logger.debug(
                f"Blob backend options: {list(self.blob_backend_options.keys())}"
            )
...
class CompressionConfig:
    ...
    enable_parallel_compression: bool = (
        True  # Use multiple threads for compression when available
    )
    compression_threshold_bytes: int = (
        1024  # Only compress objects larger than this threshold
    )
...
class CacheConfig:
    ...
    def _validate_backend_compatibility(self):
        ...
        if (
            metadata_backend in local_metadata_backends
            and blob_backend in remote_blob_backends
        ):
            raise ValueError(
...
def validate_config(config: CacheConfig) -> List["ConfigValidationError"]:
    ...
    if config.metadata.metadata_backend_options is not None:
        if not isinstance(config.metadata.metadata_backend_options, dict):
            errors.append(
                ConfigValidationError(
                    "metadata.metadata_backend_options",
    ...
    if not isinstance(config.blob.blob_backend, str):
        errors.append(
            ConfigValidationError(
                "blob.blob_backend", "must be a string", config.blob.blob_backend
            )
        )
    ...
    if config.security.custom_signed_fields:
        invalid_fields = (
            set(config.security.custom_signed_fields)
            - SecurityConfig.VALID_SIGNED_FIELDS
        )
        if invalid_fields:
            errors.append(
                ConfigValidationError(
...
def load_config_from_dict(data: dict) -> CacheConfig:
    ...
    if is_nested:
        ...
        storage = (
            CacheStorageConfig(**data.get("storage", {})) if "storage" in data else None
        )
        metadata = (
            CacheMetadataConfig(**data.get("metadata", {}))
            if "metadata" in data
            else None
        )
        blob = CacheBlobConfig(**data.get("blob", {})) if "blob" in data else None
        compression = (
            CompressionConfig(**data.get("compression", {}))
            if "compression" in data
            else None
        )
        serialization = (
            SerializationConfig(**data.get("serialization", {}))
            if "serialization" in data
            else None
        )
        handlers = (
            HandlerConfig(**data.get("handlers", {})) if "handlers" in data else None
        )
        security = (
            SecurityConfig(**data.get("security", {})) if "security" in data else None
        )
...
def save_config_to_json(
    config: CacheConfig, path: Union[str, Path], indent: int = 2
) -> None:
    """
