...
try:
    ...
    class CacheEntry(SQLModel, table=True):
        ...
        data_type: str = Field(max_length=20, index=True)  # "dataframe", "array", "object" - indexed for filtering
        prefix: str = Field(default="", max_length=100, index=True)  # Indexed for prefix-based queries
    ...
    class CacheStats(SQLModel, table=True):
        ...
        cache_misses: int = Field(default=0, index=True)  # Indexed for analytics queries
        last_updated: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), index=True)  # Indexed for monitoring
...
class JsonMetadataBackend(MetadataBackend):
    ...
    def get_entry(self, cache_key: str) -> Optional[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            return {
                "description": self._metadata["entries"][cache_key].get("description", ""),
    ...
    def put_entry(self, cache_key: str, entry_data: Dict[str, Any]):
        ...
        with self._lock:
            ...
            self._metadata["data_types"][cache_key] = entry_data.get("data_type", "unknown")
            self._metadata["creation_times"][cache_key] = entry_data.get("created_at", now)
            self._metadata["access_times"][cache_key] = entry_data.get("accessed_at", now)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            # Count by data type
            dataframe_count = sum(1 for dt in self._metadata.get("data_types", {}).values() 
                                 if dt == "dataframe")
            array_count = sum(1 for dt in self._metadata.get("data_types", {}).values() 
                             if dt == "array")
    ...
    def update_access_time(self, cache_key: str):
        """Update last access time for cache entry."""
        with self._lock:
            self._metadata["access_times"][cache_key] = datetime.now(timezone.utc).isoformat()
            self._save_to_disk()
    ...
    def cleanup_expired(self, ttl_hours: int) -> int:
        ...
        with self._lock:
            ...
            for cache_key, creation_time_str in self._metadata["creation_times"].items():
                try:
...
class SQLiteMetadataBackend(MetadataBackend):
    ...
    def __init__(self, db_file: str = "cache_metadata.db", echo: bool = False):
        ...
        if not SQLMODEL_AVAILABLE:
            raise ImportError("SQLModel is required for SQLite backend. Install with: pip install sqlmodel")
    ...
    def load_metadata(self) -> Dict[str, Any]:
        ...
        with Session(self.engine) as session:
            ...
            for entry in entries:
                ...
                metadata["access_times"][entry.cache_key] = entry.accessed_at.isoformat()
                metadata["creation_times"][entry.cache_key] = entry.created_at.isoformat()
    ...
    def list_entries(self) -> List[Dict[str, Any]]:
        """List all cache entries with metadata."""
        with Session(self.engine) as session:
            entries = session.exec(select(CacheEntry).order_by(desc(CacheEntry.created_at))).all()
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with Session(self.engine) as session:
            ...
            dataframe_count = len([e for e in total_entries if e.data_type == "dataframe"])
            ...
            # Get hit/miss stats
            stats = self._get_stats_row(session)
            hit_rate = stats.cache_hits / (stats.cache_hits + stats.cache_misses) if (stats.cache_hits + stats.cache_misses) > 0 else 0.0
...
def create_metadata_backend(backend_type: str = "auto", **kwargs) -> MetadataBackend:
    ...
    if backend_type == "json":
        ...
        if not SQLMODEL_AVAILABLE:
            raise ImportError("SQLModel is required for SQLite backend but is not available. Install with: uv add sqlmodel")
        ...
        raise ValueError(f"Unknown backend type: {backend_type}. Use 'auto', 'sqlite', or 'json'")
