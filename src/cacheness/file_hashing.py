...
def hash_directory_parallel(
    ...
    if not file_paths:
        ...
        return xxhash.xxh3_64(
            f"empty_directory:{str(directory_path)}".encode()
        ).hexdigest()
    ...
    use_parallel = max_workers > 1 and (total_size_mb > 4000 or len(file_paths) > 80)
    ...
    try:
        ...
        logger.debug(
            f"Hashed directory {directory_path} with {len(file_hashes)} files using {max_workers} workers"
        )
...
def _hash_directory_sequential(
    directory_path: Path, file_paths: List[Tuple[Path, Path]]
) -> str:
    """
