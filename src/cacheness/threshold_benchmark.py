...
class ThresholdBenchmark:
    ...
    def create_test_files(self, directory: Path, num_files: int, file_size_mb: float) -> None:
    ...
    def benchmark_scenario(self, num_files: int, file_size_mb: float, runs: int | None = None) -> Dict[str, Any]:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            print(f"   🚀 Speedup: {speedup:.2f}x, Beneficial: {'✅' if result['beneficial'] else '❌'}")
            print(f"   🔍 Hash consistent: {'✅' if result['hash_consistent'] else '❌'}")
    ...
    def run_comprehensive_benchmark(self) -> None:
        ...
        for i, (num_files, file_size_mb) in enumerate(scenarios, 1):
            try:
                ...
                print(f"📈 Running summary: {beneficial_count}/{len(self.results)} scenarios beneficial so far")
