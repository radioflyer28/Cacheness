...
def demonstrate_decorator_configs():
    ...
    fast_config = CacheConfig(
        cache_dir="./cache_fast", enable_collections=False, max_tuple_recursive_length=2
    )
...
def demonstrate_real_world_use_case():
    ...
    # Feature engineering
    start_time = time.time()
    features, target, metadata = feature_engineering(
        raw_data, transformations, hyperparams
    )
    ...
    start_time = time.time()
    features2, target2, metadata2 = feature_engineering(
        raw_data, transformations, hyperparams
    )
    ...
    print(
        f"  Results identical: {'✅' if np.array_equal(features, features2) else '❌'}"
    )
