...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        experiment_key4 = cache.put(
            ...
            model="deep_learning",
            custom_metadata=[
                experiment_metadata4,
                performance_metadata4,
            ],  # List of metadata
        )
        print(
            f"✅ Stored experiment 4 with multiple metadata types: {experiment_key4[:12]}..."
        )
        ...
        with cache.query_custom_session("experiments") as query:
            ...
            for exp in alice_experiments:
                print(
                    f"   - {exp.experiment_id}: {exp.model_type} (accuracy: {exp.accuracy})"
                )
            ...
            for exp in xgboost_models:
                print(
                    f"   - {exp.experiment_id}: epochs={exp.epochs}, accuracy={exp.accuracy}"
                )

            # Query 4: Complex query - models with epochs > 0 and accuracy > 0.9
            trained_high_acc = query.filter(
                ExperimentMetadata.epochs > 0, ExperimentMetadata.accuracy > 0.9
            ).all()
            print(
            ...
            for exp in trained_high_acc:
                print(
                    f"   - {exp.experiment_id}: {exp.model_type} ({exp.epochs} epochs, {exp.accuracy} accuracy)"
                )

        # Query 5: Query performance metadata
        with cache.query_custom_session("performance") as perf_query:
            long_running = perf_query.filter(
                PerformanceMetadata.training_time_seconds > 3600
            ).all()
            print(
            ...
            for perf in long_running:
                print(
                    f"   - {perf.run_id}: {perf.training_time_seconds}s, {perf.memory_usage_mb:.1f}MB"
                )
        ...
        param_cache.put(
            "test_model_1",
            experiment="param_exp_1",
            model_type="xgboost",
            accuracy=0.95,
        )
        param_cache.put(
            "test_model_2", experiment="param_exp_2", model_type="cnn", accuracy=0.88
        )
        param_cache.put(
            "test_model_3",
            experiment="param_exp_3",
            model_type="random_forest",
            accuracy=0.92,
        )
        ...
        if all_param_entries:
            print(
                f"✅ Found {len(all_param_entries)} total entries with stored parameters"
            )
        ...
        if xgboost_entries:
            ...
            for entry in xgboost_entries:
                params = entry.get("cache_key_params", {})
                print(
                    f"   - {entry['cache_key'][:12]}... (experiment: {params.get('experiment', 'N/A')})"
                )
        else:
        ...
        if specific_exp:
            ...
            for entry in specific_exp:
                params = entry.get("cache_key_params", {})
                print(
                    f"   - {entry['description']} (model: {params.get('model_type', 'N/A')})"
                )
        else:
