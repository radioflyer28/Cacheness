...
**Architecture:** When `storage_mode=True`, `put()`, `get()`, and
`get_with_metadata()` delegate to dedicated passthrough methods that bypass
cache concerns entirely. This is implemented via early-return at the top of
each method — the normal cache path (TTL checks, stats, eviction, auto-delete)
is never reached.
