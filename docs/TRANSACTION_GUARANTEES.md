...
**Key invariant:** A blob without metadata is a harmless orphan (cleaned up by `verify_integrity`). Metadata pointing to a missing blob is a **dangling pointer** — the dangerous failure mode. The blob-first ordering prevents dangling pointers during normal operation.
...
**Self-healing:** If step 2 fails (blob missing or corrupt), `get()` automatically deletes the metadata entry and returns `None`. This is the primary recovery mechanism for dangling pointers. The exception is transient I/O errors, which are re-raised without deleting.
...
**Failure window:** Between steps 2 and 3, a crash leaves metadata pointing to a deleted blob (dangling pointer). The next `get()` call will self-heal by removing the stale metadata.
...
| Crash during `delete()` step 2 (blob delete) | Blob gone, metadata still present | `get()` self-heals (removes stale metadata) |
