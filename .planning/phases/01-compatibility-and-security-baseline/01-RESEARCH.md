...
- Git history contains identifiable release commits for `"0.3.7"`, `"0.3.9"`, `"0.3.13"`, and current `"0.3.14"`; these span pre-signing, pre-denormalized SQLite metadata, pre-unified cache keying, and current layouts. Check in artifacts produced once from those commits, then make tests consume immutable fixtures rather than installing historical packages at test time. [VERIFIED: git history commits a756d70,6e2f993,76a469f,8be1f66] [ASSUMED]
...
For filesystem operations, preserve the authored configuration string and resolve the root only at runtime. Resolve the root itself first (thereby allowing a configured root symlink), reject absolute/POSIX traversal/Windows drive/UNC/rooted forms, enforce a restricted opaque-ID grammar, construct the sharded path, reject every symlink below the resolved root, and finally prove containment. `Path.resolve()` eliminates `".."` and follows symlinks; `Path.is_relative_to()` alone is lexical and does not treat `".."` specially. [CITED: https://docs.python.org/3.11/library/pathlib.html]
...
**What goes wrong:** A managed path component can be retargeted after validation but before access. [ASSUMED]

**How to avoid:** At minimum, reject all existing symlink components immediately before access and keep path construction/access centralized. If tests demonstrate a hostile concurrent-retarget threat, use descriptor-relative/no-follow OS primitives where supported rather than claiming `resolve()` alone eliminates TOCTOU. [ASSUMED]
...
|---|-------|---------|---------------|
| A1 | Use release commits for `0.3.7`, `0.3.9`, `0.3.13`, and `0.3.14` as the representative fixture set. | Existing Baseline | Another historical boundary may be more representative; fixture-generation reconnaissance should verify readability before locking the matrix. |
| A2 | Add one shared internal module at `src/cacheness/storage/path_security.py` and focused new test files. | Recommended Project Structure | Existing module placement may offer a cleaner import boundary; behavior and ownership matter more than the filename. |
| A3 | A manual fixed tuple grammar is the simplest safe parser. | Architecture Patterns | An equally bounded parser could be acceptable, but `ast.literal_eval` must not be treated as resource-safe for untrusted metadata. |
| A4 | Immediate symlink rejection plus centralized access is sufficient for Phase 1 unless adversarial concurrent retargeting is demonstrated. | Common Pitfalls | A stronger descriptor-relative design may be required for a hostile local actor; planner should add a focused threat/test checkpoint. |
| A5 | Exact new exception names, reason-code values, best-effort reporting representation, and logging field names remain planner choices. | Architecture Patterns / Code Examples | These become contractual under D-08 once selected, so lock them in the plan before implementation. |

## Open Questions

1. **Which earlier 0.3.x artifacts are safely identifiable and readable?**
   - What we know: Git history exposes distinct commits around signing, SQLite schema, and key-generation changes. [VERIFIED: git history commits a756d70,6e2f993,76a469f,8be1f66]
   - What's unclear: The exact cross-version artifact matrix has not yet been generated and read under current code.
   - Recommendation: Make fixture reconnaissance the first task: generate candidates in isolated temporary checkouts, copy only safely identifiable/readable artifacts into checked-in fixtures, and record exclusions. Do not promise unreadable formats. [ASSUMED]

2. **How strong must symlink race resistance be for the trusted local-host model?**
   - What we know: D-11 requires root symlinks to work and managed-path symlinks to fail; current code follows them. [VERIFIED: .planning/phases/01-compatibility-and-security-baseline/01-CONTEXT.md] [VERIFIED: local tempfile containment probe 2026-08-29]
   - What's unclear: Whether Phase 1 must defend against a hostile local process retargeting a path between validation and access.
   - Recommendation: Add a planning checkpoint that states the threat model. If adversarial concurrent retargeting is in scope, implement/test descriptor-relative no-follow access where supported; otherwise document the limitation without overstating `resolve()`. [ASSUMED]
...
- Pitfalls: HIGH for reproduced code paths; MEDIUM for symlink TOCTOU scope because the local hostile-process threat model is not explicitly locked.
- Validation architecture: HIGH for required behaviors and current test infrastructure; MEDIUM for proposed filenames and historical fixture selection.
