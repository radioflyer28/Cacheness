...
Only the one-run frozen non-live suite, its non-rerunning record selector, and
the separate Plan 11-10 audit derivation remain pending.
...
- [ ] TensorFlow is absent from runtime/config/exports, package metadata, lockfile, CI, current docs, and current codebase maps; qualification accepts exactly `core` and rejects both `tensorflow` and `non_tensorflow` profile inputs/rows.
