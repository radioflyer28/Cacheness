...
| Existing lifecycle authority implementations | SQLite schema `"7"`; PostgreSQL schema `"3"` | Canonical identity/revision, raw inventory, and topology-specific publication transactions | The exact persisted schema values are `SQLITE_USER_VERSION = 7` and `POSTGRESQL_AUTHORITY_SCHEMA_VERSION = 3`; authority transactions already own visibility. [VERIFIED: `src/cacheness/storage/sqlite_lifecycle_authority.py:62-79`; `src/cacheness/storage/backends/postgresql_lifecycle_authority.py:62-69`] |
...
| SQLite application metadata | `application_id = 0x43414348`, `user_version = 7` | Local authority identity and schema compatibility | Read during inspection; never rewrite during ordinary open. [VERIFIED: `src/cacheness/storage/sqlite_lifecycle_authority.py:62-79,835-928`] |
...
**How to avoid:** Retain the prior selection inside authority state; permit rollback only while offline; finalize ends rollback; purge is later, confirmed, and retryable.
...
1. **Does Phase 7 require a persisted schema change to support store-wide activation?**
   - What we know: current authorities publish entry lifecycle state and do not expose activate/rollback/finalize for a complete store candidate. [VERIFIED: `src/cacheness/storage/lifecycle_authority.py:28-190,284-383`]
   - What's unclear: whether the current authority tables can encode retained prior/current catalog selection without a new persisted table/field.
   - Recommendation: make this the first planner checkpoint. Add only the smallest schema change required by real functionality; do not bump a format merely to demonstrate migration. [VERIFIED: `.planning/phases/07-explicit-migration-and-rebuild-cutover/07-CONTEXT.md:33`]

2. **What is the exact public operator entry point?**
   - What we know: `pyproject.toml` has no `[project.scripts]` entry, and public symbol names are delegated. [VERIFIED: `pyproject.toml:1-82`; `.planning/phases/07-explicit-migration-and-rebuild-cutover/07-CONTEXT.md:58-59`]
   - What's unclear: Python API only versus a thin CLI over the same models.
   - Recommendation: implement the library API first and, if a CLI is included, keep it a stateless adapter that requires explicit paths/run IDs and renders the same plan model.

3. **How should stopped-worker status be evidenced?**
   - What we know: online writer coordination is out of scope; every mutation detects stale source revision and fails. [VERIFIED: `.planning/phases/07-explicit-migration-and-rebuild-cutover/07-CONTEXT.md:39`; `.planning/ROADMAP.md:423-432`]
   - What's unclear: whether the public API requires an explicit operator acknowledgement in addition to revision checks.
   - Recommendation: require explicit acknowledgement in each mutating command/report, but do not add a lease, lock server, or attempt to prove global quiescence.

4. **Which handler transformation API shape should be public?**
   - What we know: current handlers expose exact-contract support only; no directed source-to-target transformation edge exists. [VERIFIED: `src/cacheness/interfaces.py:120-148`; `src/cacheness/handlers.py:1437-1473`]
   - What's unclear: method on handlers versus a separate registry keyed by handler and payload identities.
   - Recommendation: keep ownership with the registered handler contract and require explicit directed edges; do not centralize native conversions in migration code.
