...
- `src/cacheness/storage/blob_store.py`: `put_entry`, `get_entry_info`, `open_entry`,
  `get_metadata`, `update_metadata`, and `list` already expose the same-generation
  BlobStore catalog seam. Extend these compatible surfaces rather than inventing an
  independent catalog coordinator.
- `src/cacheness/storage/backends/__init__.py`: metadata and blob registries already
  support named construction. Route them through the new composition root while
  preserving registration compatibility.
...
- Replace `BlobStore._select_projection_backend()` and
  ...
  ownership-aware composition path while preserving compatible entry points.
...
- Extend entry descriptors and authority schemas to retain schema identity, declared
  ...
  payload bytes.
- Replace `BlobStore.list(..., metadata_filter=dict)` as the only direct query shape
  with a typed, cursor-based portable query surface while keeping compatible equality
  filtering.
- Keep legacy custom-metadata and SQLAlchemy session hooks behind a derived projection
