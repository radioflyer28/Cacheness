...
| Current D-24 phase-closing command | `uv run --isolated --all-extras --group dev --frozen python tools/verify_phase8_contracts.py --local-ready --output .planning/phases/08-production-gates-and-performance-stabilization/08-LOCAL-READINESS.json` |
| Preserved SEED-007 release-boundary command | `uv run --isolated --all-extras --group dev --frozen python tools/verify_phase8_contracts.py --all` — retained for future exact-SHA live-service/publication qualification; not the current Phase 8 completion command |
