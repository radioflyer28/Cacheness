...
- Signing/encryption key storage hardening — currently keys are stored as plaintext files on disk (with restrictive permissions via chmod/icacls). Harden by integrating with OS-level secure storage: Windows DPAPI / Credential Manager, macOS Keychain, Linux Secret Service (libsecret) / kernel keyring. Alternatively, support KMS-backed key wrapping (AWS KMS, Azure Key Vault, HashiCorp Vault) for server-side deployments where the key file approach is insufficient. The `keyring` Python package provides a cross-platform abstraction over OS keychains that could serve as the integration point.
- Metadata database encryption at rest — encrypt the metadata DB itself (SQLite/libSQL via SQLCipher or libSQL native encryption, PostgreSQL via pgcrypto or client-side field encryption). Important for untrusted-server scenarios where cache keys, timestamps, and data types are also sensitive. Separate from blob encryption because it's a DB-level concern, not a BlobStore-level concern. **Nuance:** Phase 18 blob encryption + existing HMAC signing (both client-side keyed) protect data *confidentiality* and *integrity* against a compromised server, but metadata remains in plaintext — a compromised server can still observe access patterns, namespaces, data types, timestamps, and custom metadata fields. Client-side keys don't help with this metadata leakage. Full zero-trust requires encrypting the metadata layer as well.
