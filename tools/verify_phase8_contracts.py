...
PHASE_DIRECTORY = ".planning/milestones/v1.0-phases/08-production-gates-and-performance-stabilization"
...
def validate_fixed_manifest(root: Path = REPOSITORY_ROOT) -> tuple[str, ...]:
    ...
    for requirement in (*PHASE8_REQUIREMENTS, *DEFERRED_REQUIREMENTS):
        if requirement not in (root / ".planning/milestones/v1.0-REQUIREMENTS.md").read_text(
            encoding="utf-8"
        ):
