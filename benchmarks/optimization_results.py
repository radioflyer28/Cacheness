...
def test_schema_optimization_performance():
    ...
    for backend_name, backend_type in backends:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print(
                f"{backend_name:26} | {put_ops_per_sec:8.0f}    | {get_ops_per_sec:8.0f}    | {list_time:8.1f}"
            )
            ...
            if len(entries) > 0:
                ...
                print(
                    f"   Metadata keys: {list(first_entry.get('metadata', {}).keys())[:5]}..."
                )  # Show first 5
            cache.close()
...
def test_memory_cache_layer_benefit():
    ...
    for config_name, enable_memory_cache in configs:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print(
                f"{config_name:26} | {get_ops_per_sec:8.0f}    | {list_time:8.1f}      | {hit_rate}"
            )
            cache.close()
