...
HISTORICAL_CONTENTION_SCHEMA_VERSION = 2
...
# latency target. Runtime timeout policy remains caller-owned.
MULTIPLIERS = {
    "busy_deadline": 2.0,
...
_HISTORICAL_CONTENTION_ENVELOPE_STAGES = (
...
def _contention_envelope_seconds(stages: dict[str, dict[str, Any]]) -> float:
    """Recompute the historical evidence envelope from preserved raw samples."""
    return (
        sum(stages[name]["p99"] for name in _HISTORICAL_CONTENTION_ENVELOPE_STAGES)
...
def derive_configuration(
    ...
    """Derive measured page-size recommendations, never runtime timeout policy."""
...
def build_baseline() -> dict[str, Any]:
    ...
    return {
        ...
        "multiplier_rationale": {
            ...
            "reconciliation_time": "2x p99 retains a benchmark-only page-time envelope.",
...
def _validate_contention(contention: object) -> None:
    """Validate historical contention evidence without deriving runtime policy."""
    ...
    historical_budget = section.get("historical_runtime_budget_capture_seconds")
    if (
        isinstance(historical_budget, bool)
        or not isinstance(historical_budget, (int, float))
        or not math.isfinite(historical_budget)
        or historical_budget <= 0
    ):
        raise BenchmarkVerificationError("contention historical budget capture is invalid")
    if section.get("classification") != "performance evidence; not runtime timeout policy":
        raise BenchmarkVerificationError("contention classification is incompatible")
...
def validate_baseline(baseline: object) -> dict[str, Any]:
    ...
    if schema_version not in {
        ...
        HISTORICAL_CONTENTION_SCHEMA_VERSION,
    ...
    if (
        schema_version == HISTORICAL_CONTENTION_SCHEMA_VERSION
        or document.get("contention") is not None
    ):
