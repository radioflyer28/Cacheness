...
@pytest.fixture
def json_cache():
    """Fixture to create a temporary cache with JSON backend for query_meta tests."""
    temp_dir = tempfile.mkdtemp()
    cache_dir = Path(temp_dir) / "cache"
...
class TestQueryMeta:
    ...
    def test_query_meta_basic_functionality(self, temp_cache):
        ...
        cache.put(
            "model_3", experiment="exp_003", model_type="random_forest", accuracy=0.92
        )
    ...
    def test_query_meta_multiple_filters(self, temp_cache):
        ...
        filtered_entries = cache.query_meta(model_type="xgboost", active=True)
    ...
    def test_query_meta_with_json_backend(self, temp_cache):
        """Test query_meta works with non-SQLite backends (generic fallback)."""
        import tempfile
        import shutil
        ...
        try:
            ...
            # query_meta should work via Python-side fallback
            result = json_cache.query_meta(experiment="exp_001")
            assert result is not None
            assert len(result) == 1
            assert result[0]["metadata_dict"]["experiment"] == "exp_001"
        finally:
            shutil.rmtree(temp_dir)
    ...
    def test_query_meta_with_different_data_types(self, temp_cache):
        ...
        temp_cache.put(np.array([1, 2, 3]), data_type="numpy", format="array")
        temp_cache.put(
...
class TestQueryMetaJsonBackend:
    """Test query_meta() with JSON metadata backend (Python-side fallback)."""

    def test_query_meta_returns_list_not_none(self, json_cache):
        """JSON backend query_meta returns a list, not None."""
        json_cache.put("data1", experiment="exp_001", model_type="xgboost")
        result = json_cache.query_meta()
        assert result is not None
        assert isinstance(result, list)

    def test_query_meta_no_filters(self, json_cache):
        """All entries returned when no filters are given."""
        json_cache.put("d1", experiment="exp_001", model_type="xgboost")
        json_cache.put("d2", experiment="exp_002", model_type="cnn")
        json_cache.put("d3", experiment="exp_003", model_type="rf")

        entries = json_cache.query_meta()
        assert len(entries) == 3
        # Verify structure
    ...
    def test_query_meta_string_filter(self, json_cache):
        """String filters work correctly on JSON backend."""
        json_cache.put("d1", experiment="exp_001", model_type="xgboost")
        json_cache.put("d2", experiment="exp_002", model_type="cnn")
        json_cache.put("d3", experiment="exp_003", model_type="xgboost")

        entries = json_cache.query_meta(model_type="xgboost")
        assert len(entries) == 2
        for e in entries:
            assert e["metadata_dict"]["model_type"] == "xgboost"

    def test_query_meta_numeric_filter(self, json_cache):
        """Numeric filters work correctly on JSON backend."""
        json_cache.put("d1", experiment="exp_001", accuracy=0.95)
        json_cache.put("d2", experiment="exp_002", accuracy=0.88)

        entries = json_cache.query_meta(accuracy=0.95)
        assert len(entries) == 1
        assert entries[0]["metadata_dict"]["accuracy"] == 0.95

    def test_query_meta_multiple_filters(self, json_cache):
        """Multiple filters are AND-ed together."""
        json_cache.put("d1", model_type="xgboost", version="v1")
        json_cache.put("d2", model_type="cnn", version="v1")
        json_cache.put("d3", model_type="xgboost", version="v2")

        entries = json_cache.query_meta(model_type="xgboost", version="v1")
        assert len(entries) == 1
        assert entries[0]["metadata_dict"]["model_type"] == "xgboost"
        assert entries[0]["metadata_dict"]["version"] == "v1"

    def test_query_meta_no_match(self, json_cache):
        """Returns empty list when no entries match."""
        json_cache.put("d1", model_type="xgboost")
        entries = json_cache.query_meta(model_type="nonexistent")
        assert entries is not None
        assert len(entries) == 0

    def test_query_meta_entry_structure(self, json_cache):
        """Returned entries have correct structure matching SQLite path."""
        json_cache.put("data", experiment="exp_001", accuracy=0.9)

        entries = json_cache.query_meta(experiment="exp_001")
        assert len(entries) == 1
        entry = entries[0]
        ...
        assert "metadata_dict" in entry
        assert isinstance(entry["metadata_dict"], dict)
        assert entry["metadata_dict"]["experiment"] == "exp_001"

    def test_query_meta_requires_store_full_metadata(self):
        """Returns None when store_full_metadata is False."""
        temp_dir = tempfile.mkdtemp()
        try:
            config = CacheConfig(
            ...
            cache = UnifiedCache(config)
            result = cache.query_meta(foo="bar")
            assert result is None
            cache.close()
        finally:
            import shutil
    ...
    def test_delete_matching_works_with_json_backend(self, json_cache):
        """delete_matching uses query_meta / generic path correctly on JSON backend."""
        json_cache.put("d1", project="alpha", model="xgb")
        json_cache.put("d2", project="beta", model="cnn")
        json_cache.put("d3", project="alpha", model="rf")

        # delete_matching should work for JSON via generic path
        deleted = json_cache.delete_matching(project="alpha")
        assert deleted == 2

        # Only beta remains
        remaining = json_cache.query_meta()
        assert remaining is not None
        assert len(remaining) == 1
        assert remaining[0]["metadata_dict"]["project"] == "beta"
