...
PHASE3_VERIFICATION = (
    ROOT
    / ".planning/milestones/v1.0-phases/03-atomic-lifecycle-and-recovery-engine/03-VERIFICATION.md"
)
PHASE8_VALIDATION = (
    ROOT
    / ".planning/milestones/v1.0-phases/08-production-gates-and-performance-stabilization/08-VALIDATION.md"
)
PHASE3_VALIDATION = (
    ROOT
    / ".planning/milestones/v1.0-phases/03-atomic-lifecycle-and-recovery-engine/03-VALIDATION.md"
)
PHASE11_VALIDATION = (
    ROOT
    / ".planning/milestones/v1.0-phases/11-clean-supplemental-documentation-and-normalize-validation-ev/11-VALIDATION.md"
)
MILESTONE_AUDIT = ROOT / ".planning/milestones/v1.0-MILESTONE-AUDIT.md"
...
CANONICAL_VALIDATIONS = (
    (
        "01",
        ROOT / ".planning/milestones/v1.0-phases/01-compatibility-and-security-baseline/01-VALIDATION.md",
    ),
    ("03", PHASE3_VALIDATION),
    (
        "05",
        ROOT
        / ".planning/milestones/v1.0-phases/05-payload-backends-and-supported-topology-qualification/05-VALIDATION.md",
    ),
    (
        "06",
        ROOT / ".planning/milestones/v1.0-phases/06-unifiedcache-policy-composition/06-VALIDATION.md",
    ),
    (
        "07",
        ROOT / ".planning/milestones/v1.0-phases/07-explicit-migration-and-rebuild-cutover/07-VALIDATION.md",
    ),
    ("08", PHASE8_VALIDATION),
    (
        "09",
        ROOT / ".planning/milestones/v1.0-phases/09-adoption-and-release-surface-closure/09-VALIDATION.md",
    ),
)
EXPLICIT_NONCLAIMS = (
    "PostgreSQL",
...
def _assert_archived_qualified_source_tree(repository: Path, revision: str) -> None:
    """Check the historical v1.0 tree, not later work on the live checkout."""

    assert re.fullmatch(r"[0-9a-f]{40}", revision)
    assert _git_output(repository, "rev-parse", f"{revision}^{{commit}}") == revision
    assert _git_output(repository, "merge-base", "--is-ancestor", revision, "v1.0") == ""
    changed = _git_output(repository, "diff", "--name-only", f"{revision}..v1.0")
    protected = [
        path
        for path in changed.splitlines()
        if _is_protected_tree_path(path) and path != ".planning/REQUIREMENTS.md"
    ]
    assert not protected, protected
    assert (repository / ".planning/milestones/v1.0-REQUIREMENTS.md").is_file()
...
def test_phase11_validation_record_matches_final_acceptance_evidence() -> None:
    ...
    if status == "draft":
        ...
        if qualified_source_revision is not None:
            _assert_archived_qualified_source_tree(ROOT, qualified_source_revision)
    else:
