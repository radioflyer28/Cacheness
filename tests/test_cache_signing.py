...
class TestDeleteInvalidSignatures:
    ...
    def test_config_logging(self):
        ...
        try:
            ...
            assert (
                "delete_invalid=False" in log_output
                or "delete_invalid_signatures" in str(config.security.__dict__)
            )
    ...
    def test_in_memory_key_functionality(self):
        ...
        try:
            ...
            config_persistent = CacheConfig(
                ...
                security=SecurityConfig(
                    enable_entry_signing=True, use_in_memory_key=False
                ),
            )
            ...
            config_memory = CacheConfig(
                ...
                security=SecurityConfig(
                    enable_entry_signing=True, use_in_memory_key=True
                ),
            )
            ...
            assert not key_file_path_memory.exists(), (
                "Key file should NOT be created for in-memory keys"
            )

            # Test basic functionality
            test_data = {"test": "data", "value": 42}
            cache_key = cache_memory.put(
                test_data, description="Test with in-memory key"
            )
    ...
    def test_in_memory_key_restart_behavior(self):
        ...
        try:
            ...
            assert retrieved2 is None, (
                "Data should not be accessible after restart with in-memory key"
            )
