...
class TestHashDirectoryParallel:
    ...
    def test_empty_directory(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            expected = xxhash.xxh3_64(f"empty_directory:{str(temp_path)}".encode()).hexdigest()
    ...
    def test_nested_directory_structure(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (temp_path / "dir2" / "subdir" / "file2.txt").write_bytes(b"Deeply nested file")
    ...
    def test_small_directory_uses_sequential(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('cacheness.file_hashing._hash_directory_sequential') as mock_sequential:
    ...
    def test_large_directory_uses_parallel(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('cacheness.file_hashing.ProcessPoolExecutor') as mock_executor_class:
                ...
                with patch('cacheness.file_hashing.as_completed', return_value=mock_futures):
                    result = hash_directory_parallel(temp_path)
    ...
    def test_parallel_processing_fallback(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('cacheness.file_hashing.ProcessPoolExecutor', side_effect=Exception("Process pool failed")):
                with patch('cacheness.file_hashing._hash_directory_sequential') as mock_sequential:
    ...
    def test_file_hashing_error_handling(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('cacheness.file_hashing._hash_single_file', side_effect=mock_hash_with_errors):
                result = hash_directory_parallel(temp_path)
...
class TestParallelHashingLegacy:
    """Test cases for parallel directory hashing functionality (migrated from test_parallel_path_hashing.py)."""
    ...
    def test_hash_file_content_consistency(self):
        """Test single file content hashing consistency."""
        with tempfile.TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            test_file = tmp_path / "test.txt"
