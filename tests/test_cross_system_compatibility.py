...
class TestCrossSystemCompatibility:
    ...
    def test_same_cache_keys_for_equivalent_data(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            decorator_key = _generate_cache_key(
                test_function, args, kwargs, config=config
            )
            ...
            assert decorator_key == unified_key, (
                f"Keys should match: decorator={decorator_key}, unified={unified_key}"
            )
    ...
    def test_decorator_cache_unified_retrieve(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            normalized_params = _normalize_function_args(
                compute_result, (10, 20), {"operation": "add"}
            )
            ...
            assert cached_data == 30, (
                f"UnifiedCache should retrieve decorator's cached data: {cached_data}"
            )
    ...
    def test_unified_cache_decorator_retrieve(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            normalized_params = _normalize_function_args(
                process_data, (test_items,), {"sort": True, "limit": 5}
            )
            ...
            assert cached_result == expected_result, (
                f"Decorator should retrieve UnifiedCache's data: {cached_result}"
            )
    ...
    def test_cross_compatibility_with_complex_data(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            decorator_result = decorated_matrix_op(
                test_array, operation="transpose", scale=2.0
            )
            ...
            normalized_params = _normalize_function_args(
                decorated_matrix_op,
                (test_array,),
                {"operation": "transpose", "scale": 2.0},
            )
            func_id = (
                f"{decorated_matrix_op.__module__}.{decorated_matrix_op.__qualname__}"
            )
            enhanced_params = {**normalized_params, "__function__": func_id}
            cache_key = cache_instance._create_cache_key(enhanced_params)
            ...
            np.testing.assert_array_equal(
                unified_result,
                expected,
                "UnifiedCache should retrieve decorator's NumPy data",
            )
    ...
    def test_cross_compatibility_with_dataframes(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            normalized_params = _normalize_function_args(
                process_dataframe, (test_df,), {"column": "A", "operation": "sum"}
            )
            ...
            assert cached_result == expected_result, (
                f"Decorator should retrieve DataFrame processing result: {cached_result}"
            )
    ...
    def test_parameter_normalization_consistency(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i, (args, kwargs) in enumerate(test_cases):
                ...
                decorator_key = _generate_cache_key(
                    multi_param_function, args, kwargs, config=config
                )

                # Generate key using UnifiedCache method
                normalized_params = _normalize_function_args(
                    multi_param_function, args, kwargs
                )
                ...
                assert decorator_key == unified_key, (
                    f"Test case {i}: Keys should match for args={args}, kwargs={kwargs}"
                )
    ...
    def test_cache_key_stability_across_systems(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            normalized_params = _normalize_function_args(
                cached_stable_function, (test_value,), {"multiplier": 2}
            )
            ...
            assert result3 == expected_result, (
                "Original decorator cache should still work"
            )
    ...
    def test_different_functions_different_keys(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert key_a != key_b, (
                f"Different functions should have different keys: {key_a} vs {key_b}"
            )
            ...
            assert unified_key_a != unified_key_b, (
                "UnifiedCache should also generate different keys for different functions"
            )
            assert key_a == unified_key_a, (
                "Decorator and UnifiedCache should generate same key for function_a"
            )
            assert key_b == unified_key_b, (
                "Decorator and UnifiedCache should generate same key for function_b"
            )
