...
def cache_with_backend(
    request,
    temp_cache_dir,
    postgres_available,
    experiment_metadata_model,
    performance_metadata_model,
):
    """
...
class TestBasicStorageRetrieval:
    ...
    def test_store_single_custom_metadata(
    ...
    def test_store_multiple_custom_metadata(
        ...
        experiment = experiment_metadata_model(
            experiment_id="exp_002", model_type="lightgbm", accuracy=0.92, epochs=50
        )
        ...
        cache.put(
            test_data, experiment="exp_002", custom_metadata=[experiment, performance]
        )
...
class TestQueryFunctionality:
    ...
    def test_query_custom_session_basic(
        self, cache_with_backend, experiment_metadata_model
    ):
    ...
    def test_query_with_ordering(self, cache_with_backend, experiment_metadata_model):
        ...
        with cache.query_custom_session("test_experiments") as query:
            ordered = query.order_by(experiment_metadata_model.accuracy.desc()).all()
...
class TestForeignKeyConstraints:
    ...
    def test_cascade_delete_on_cache_entry_removal(
        ...
        experiment = experiment_metadata_model(
            experiment_id="exp_cascade", model_type="xgboost", accuracy=0.95, epochs=100
        )
        cache.put(
            {"data": "test"}, experiment="exp_cascade", custom_metadata=experiment
        )
        ...
        custom_meta_after = cache.get_custom_metadata_for_entry(
...
class TestMultipleMetadataModels:
    ...
    def test_multiple_models_coexist(
        ...
        exp1 = experiment_metadata_model(
            experiment_id="exp_multi_1", model_type="xgboost", accuracy=0.95, epochs=100
        )
        ...
        perf2 = performance_metadata_model(
            run_id="run_multi_2", training_time_seconds=150.0, memory_usage_mb=2048.0
        )
        ...
        exp3 = experiment_metadata_model(
            experiment_id="exp_multi_3", model_type="lightgbm", accuracy=0.92, epochs=50
        )
        perf3 = performance_metadata_model(
            run_id="run_multi_3", training_time_seconds=100.0, memory_usage_mb=1024.0
        )
        cache.put(
            {"data": "model_3"}, experiment="exp_multi_3", custom_metadata=[exp3, perf3]
        )
...
class TestDifferentColumnTypes:
    ...
    def test_various_column_types(self, cache_with_backend):
        ...
        stored_dt = (
            stored.datetime_field.replace(tzinfo=timezone.utc)
            if stored.datetime_field.tzinfo is None
            else stored.datetime_field
        )
        assert (
            abs((stored_dt - test_datetime).total_seconds()) < 1
...
class TestEdgeCasesErrorHandling:
    ...
    def test_duplicate_unique_constraint(
        self, cache_with_backend, experiment_metadata_model
    ):
    ...
    def test_custom_metadata_without_cache_entry(
        self, cache_with_backend, experiment_metadata_model
    ):
        ...
        # Store properly
        experiment = experiment_metadata_model(
            experiment_id="exp_valid", model_type="xgboost", accuracy=0.95, epochs=100
        )
...
class TestCoreIntegration:
    ...
    def test_custom_metadata_not_used_in_cache_key_generation(
        ...
        exp1 = experiment_metadata_model(
            experiment_id="exp_key_1", model_type="xgboost", accuracy=0.95, epochs=100
        )
        ...
        key1 = cache.put(
            {"data": "test_1"}, experiment="same_key", custom_metadata=exp1
        )
        key2 = cache.put(
            {"data": "test_2"}, experiment="same_key", custom_metadata=exp2
        )
    ...
    def test_custom_metadata_not_used_in_cache_hit_logic(
        ...
        # Store entry with custom metadata
        experiment = experiment_metadata_model(
            experiment_id="exp_hit", model_type="xgboost", accuracy=0.95, epochs=100
        )
        cache.put(
            {"data": "original"}, experiment="hit_test", custom_metadata=experiment
        )
...
class TestBackendParity:
    ...
    def test_same_behavior_both_backends(
        self, cache_with_backend, experiment_metadata_model
    ):
        ...
        # Store data
        experiment = experiment_metadata_model(
            experiment_id="exp_parity", model_type="xgboost", accuracy=0.95, epochs=100
        )
        cache.put(
            {"data": "test"}, experiment="parity_test", custom_metadata=experiment
        )
